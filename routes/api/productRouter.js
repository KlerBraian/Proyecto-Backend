...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const productsDb = await productServiceMongo.getProducts(filter)
        if (sort === "asc") {
        ...
        res.send({
            status: "success", payload: paginatedProducts, totalPages: totalPages, prevPage: prevPage, nextPage: nextPage, page: pageInt, hasPrevPage: hasPrevPage, hasNextPage: hasNextPage, prevLink: prevLink, nextLink: nextLink
        });


    } catch (error) {
        res.status(500).send({
            status: "error", message: "Hubo un error al obtener los productos"
        });
    }
});
